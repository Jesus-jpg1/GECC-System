...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('username', type=str, help='O nome de usuário da Unidade Demandante que será a criadora dos editais.')
        
        # Argumento opcional para a quantidade
        parser.add_argument('--total', type=int, default=5, help='Indica o número de editais a serem criados.')
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            if user.servidorprofile.funcao != 'Unidade Demandante':
                raise CommandError(f'O usuário "{username}" não tem a função de Unidade Demandante.')
        except User.DoesNotExist:
            raise CommandError(f'Usuário "{username}" não encontrado.')

        self.stdout.write(self.style.NOTICE(f'Criando {total} editais de teste para o usuário "{username}"...'))
