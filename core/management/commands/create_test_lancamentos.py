...
from core.models import Edital, User, LancamentoHoras
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=15,
            help="Indica o número de lançamentos de horas a serem criados.",
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        editais_com_atividades = Edital.objects.filter(
        ...
        if not servidores.exists():
            self.stdout.write(
                self.style.ERROR(
                    'Nenhum usuário com a função "Servidor" encontrado. Crie alguns com o comando "create_test_users".'
                )
            )
            return
        ...
        if not editais_com_atividades.exists():
            self.stdout.write(
                self.style.ERROR(
                    "Nenhum edital com atividades associadas encontrado. Crie editais e adicione atividades primeiro."
                )
            )
            return

        self.stdout.write(
            self.style.NOTICE(f"Criando {total} lançamentos de horas de teste...")
        )
        ...
        for _ in range(total):
            ...
            data_aleatoria = timezone.now().date() - timedelta(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"{lancamentos_criados} lançamentos de horas criados com sucesso!"
            )
        )
